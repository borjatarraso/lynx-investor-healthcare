...
from lynx_health.export import export_report, ExportFormat


@pytest.fixture
def sample_report():
    p = CompanyProfile(ticker="TEST", name="Test Biopharma Corp",
    ...
    r = AnalysisReport(
        profile=p,
        valuation=ValuationMetrics(pb_ratio=5.0, cash_to_market_cap=0.40, ev_revenue=6.0,
                                   ev_gross_profit=9.0),
        solvency=SolvencyMetrics(cash_runway_years=3.0, total_cash=200_000_000,
                                 current_ratio=4.0, debt_to_equity=0.1),
        growth=GrowthMetrics(shares_growth_yoy=0.08, revenue_growth_yoy=0.35,
                             rd_intensity=0.65),
        health_quality=HealthQualityIndicators(quality_score=72.0,
...
class TestTxtExport:
    def test_contains_company_name(self, sample_report):
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as f:
            p = export_report(sample_report, "txt", Path(f.name))
            content = p.read_text()
            assert "Test Biopharma Corp" in content

    def test_contains_stage(self, sample_report):
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as f:
            content = export_report(sample_report, "txt", Path(f.name)).read_text()
            assert "Clinical" in content or "Growth" in content or "Hyper-Growth" in content
