"""Rich console display for Healthcare analysis reports — stage and tier aware.

All metric sections use the 5-level relevance system driven by get_relevance()
with BOTH tier AND healthcare lifecycle stage parameters:
...
def _a_pe(val):
    ...
    if val < 12:
        return "[green]Very cheap — possible value or distressed healthcare[/]"
    if val < 18:
...
def _a_ps(val):
    ...
    if val < 2.0:
        return "[green]Cheap — typical of commercial cash-generative pharma/medtech[/]"
    if val < 4.0:
...
def _a_divy(val):
    if _isna(val):
        return "[dim]No dividend — typical for growth healthcare[/]"
    if val == 0:
...
def _a_ctm(val):
    ...
    if val > 0.05:
        return "[#ff8800]Low cash backing — financing risk for clinical-stage biotech[/]"
...
def _a_roa(val):
    ...
    if val < 0.08:
        return "[green]Strong ROA for asset-heavy healthcare (hospitals, diagnostics, distribution)[/]"
...
def _a_dil(val):
    ...
    if val < 0.05:
        return "[yellow]Modest dilution — typical of funded growth healthcare[/]"
    if val < 0.10:
...
def display_full_report(report):
    ...
    if report.health_quality:
        _display_health_quality(report)
...
def display_report_stage(stage, report):
    ...
    handlers = {
        ...
        "health_quality": lambda: (
            _progressive_stages_seen.add("health_quality"),
            _display_health_quality(report),
        ),
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Healthcare Analysis[/]",
        border_style="blue",
    ))
    ...
    console.print(Panel(
        ...
        f"[cyan]Healthcare Category:[/] {p.health_category.value}  |  "
...
def _display_valuation(report):
    ...
    # healthcare-specific valuation rows ----------------------------------
...
def _display_profitability(report):
    ...
    if all_irrelevant:
        ...
        # healthcare-specific profitability rows ----------------------------------
...
def _display_solvency(report):
    ...
    # healthcare-specific solvency rows
    ...
    if s.capex_to_revenue is not None:
        ...
        msg = ("Asset-light (biotech / health IT)" if ctr < 0.03
               else "Typical pharma / devices" if ctr < 0.07
               else "Capex-meaningful (hospitals, tools, diagnostics)" if ctr < 0.15
               else "Capex-heavy (hospital operator, biologics manufacturing)")
...
def _display_growth(report):
    ...
    # healthcare-specific growth metrics
...
def _display_health_quality(report):
    """Healthcare quality assessment — uses relevance markers via get_relevance with category='health_quality'."""
    m = report.health_quality
    if m is None:
        return
    tier, stage = report.profile.tier, report.profile.stage
    rel = lambda key: get_relevance(key, tier, "health_quality", stage)

    t = Table(title="Healthcare Quality Assessment", show_lines=True, border_style="bold yellow")
...
def _add_quality_row(table, label, value, relevance):
    """Add a row to the healthcare quality table with relevance-based styling."""
    if relevance == Relevance.IRRELEVANT:
        return
    style, prefix = _STYLE[relevance]
    sl = f"{prefix}[{style}]{label}[/]" if style else f"{prefix}{label}"
...
def _display_conclusion(report):
    ...
    for cat in ("valuation", "profitability", "solvency", "growth", "health_quality"):
        t.add_row(
            cat.replace("_", " ").title(),
...
def _display_screening(c):
    ...
    t = Table(title="Healthcare Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    # --- Healthcare Benchmark & Sector Context ---
    if mi.benchmark_name or mi.sector_etf_name:
        t = Table(title="Healthcare Benchmark & Sector Context", show_lines=True, border_style="cyan")
        t.add_column("Indicator", style="bold", min_width=22, no_wrap=True)
    ...
    # --- Healthcare Investment Disclaimers ---
    if mi.disclaimers:
        console.print(Panel(
            "\n".join(f"[dim]{d}[/]" for d in mi.disclaimers),
            title="[dim]Healthcare Investment Disclaimers[/]",
            border_style="dim",
        ))
