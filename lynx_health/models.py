"""Data models for Lynx Healthcare — healthcare-focused fundamental analysis."""
...
# ---------------------------------------------------------------------------
# Healthcare company lifecycle stage classification
# ---------------------------------------------------------------------------
# Healthcare-specific stages map the clinical & commercial trajectory:
#   STARTUP  — Preclinical / discovery-stage biotech, pre-revenue
#   GROWTH   — Clinical-stage (Phase I/II/III) or early commercial, hyper-growth
#   SCALE    — Launch / scale-up of first approved products, ramping revenue
#   MATURE   — Commercial / cash-generative — multiple approved products, profitable
#   PLATFORM — Diversified healthcare franchise (Big Pharma, integrated insurers,
#              medical-device conglomerates) with durable blockbuster moats
# Enum values are kept stable for backward-compat with the shared Lynx core.

class CompanyStage(str, Enum):
    STARTUP = "Preclinical / Early-Stage Biotech"
    GROWTH = "Clinical / Hyper-Growth"
    SCALE = "Launch / Scale-Up"
    MATURE = "Commercial / Cash-Generative"
    PLATFORM = "Diversified Healthcare Franchise"


# ---------------------------------------------------------------------------
# Healthcare sub-sector classification (replaces Commodity / TechCategory)
# ---------------------------------------------------------------------------

class HealthCategory(str, Enum):
    BIG_PHARMA = "Large Pharmaceuticals"
    BIOTECH = "Biotechnology"
    GENERIC_PHARMA = "Generic & Specialty Pharma"
    MEDICAL_DEVICES = "Medical Devices"
    HEALTH_EQUIPMENT = "Healthcare Equipment & Supplies"
    HEALTH_PROVIDERS = "Healthcare Providers & Services"
    HEALTH_INSURANCE = "Managed Care / Health Insurance"
    LIFE_SCIENCES_TOOLS = "Life Sciences Tools & Diagnostics"
    HEALTH_DISTRIBUTION = "Healthcare Distribution"
...
# ---------------------------------------------------------------------------
# Regulatory / jurisdiction tiering
# Healthcare lens: FDA / EMA / PMDA approval framework quality, drug pricing
# stability, patent & data exclusivity protection, reimbursement reliability.
# ---------------------------------------------------------------------------

class JurisdictionTier(str, Enum):
    TIER_1 = "Tier 1 — FDA / EMA / PMDA stable regulation"
    TIER_2 = "Tier 2 — Moderate regulatory & reimbursement risk"
    TIER_3 = "Tier 3 — High regulatory, pricing or geopolitical risk"
    UNKNOWN = "Unknown"
...
class CompanyProfile:
    ...
    health_category: HealthCategory = HealthCategory.OTHER
...
class ProfitabilityMetrics:
    ...
    # healthcare-specific profitability
...
class EfficiencyMetrics:
    ...
    # healthcare-specific efficiency
...
@dataclass
class HealthQualityIndicators:
    """Quality scoring specific to healthcare companies."""
...
class MarketIntelligence:
    ...
    # Healthcare benchmark context (QQQ / IGV / SMH replaces commodity context)
...
class FinancialStatement:
    ...
    # healthcare-specific financial line items
...
class AnalysisReport:
    ...
    health_quality: Optional[HealthQualityIndicators] = None
...
# ---------------------------------------------------------------------------
# Stage classification helpers (healthcare lifecycle)
# ---------------------------------------------------------------------------

_STAGE_KEYWORDS = {
    CompanyStage.PLATFORM: [
        "diversified pharmaceutical", "global pharmaceutical", "diversified healthcare",
        "integrated healthcare", "multinational pharmaceutical", "blockbuster portfolio",
        "therapeutic franchise", "managed care leader",
    ],
    CompanyStage.MATURE: [
        "commercial stage", "approved products", "free cash flow positive", "buyback",
        "dividend", "cash-generative", "established pharmaceutical", "profitable",
        "commercialized therapies", "marketed drugs",
    ],
    CompanyStage.SCALE: [
        "commercial launch", "recently approved", "scaling commercial", "first approval",
        "product launch", "launch phase", "ramping sales",
    ],
    CompanyStage.GROWTH: [
        "phase 3", "phase iii", "pivotal trial", "late-stage clinical",
        "biologics license application", "nda", "bla filing", "hyper-growth",
    ],
    CompanyStage.STARTUP: [
        "preclinical", "discovery stage", "ind-enabling", "phase 1", "phase i",
        "pre-revenue", "clinical-stage biotechnology", "early-stage biotech",
        "development-stage", "investigational therapy",
    ],
}


_CATEGORY_KEYWORDS = {
    HealthCategory.BIG_PHARMA: [
        "pharmaceutical company", "pharmaceuticals", "drug manufacturer",
        "multinational pharmaceutical", "big pharma", "branded drug",
        "prescription pharmaceuticals",
    ],
    HealthCategory.BIOTECH: [
        "biotechnology", "biotech", "biologics", "gene therapy", "cell therapy",
        "mrna", "rna therapeutics", "monoclonal antibody", "antibody-drug conjugate",
        "crispr", "gene editing", "targeted therapy", "novel therapeutics",
        "clinical-stage", "preclinical",
    ],
    HealthCategory.GENERIC_PHARMA: [
        "generic pharmaceutical", "generics", "biosimilar", "specialty pharma",
        "branded generic", "authorized generic",
    ],
    HealthCategory.MEDICAL_DEVICES: [
        "medical device", "surgical instruments", "orthopedic implants",
        "cardiovascular devices", "neuromodulation", "interventional devices",
        "minimally invasive", "implantable", "stent", "pacemaker", "robotic surgery",
    ],
    HealthCategory.HEALTH_EQUIPMENT: [
        "healthcare equipment", "hospital equipment", "medical supplies",
        "infusion pumps", "patient monitoring", "disposables",
    ],
    HealthCategory.HEALTH_PROVIDERS: [
        "hospital", "health system", "healthcare services", "outpatient",
        "ambulatory", "dialysis", "physician services", "home health",
        "long-term care", "nursing facility", "clinic operator",
    ],
    HealthCategory.HEALTH_INSURANCE: [
        "managed care", "health insurance", "health insurer", "medicare advantage",
        "medicaid managed", "pbm", "pharmacy benefit", "medical loss ratio",
        "payer", "hmo", "ppo",
    ],
    HealthCategory.LIFE_SCIENCES_TOOLS: [
        "life sciences tools", "bioprocessing", "laboratory equipment",
        "diagnostics", "molecular diagnostics", "in vitro diagnostics",
        "genomic sequencing", "next-generation sequencing", "mass spectrometry",
        "dna synthesis", "clinical laboratory",
    ],
    HealthCategory.HEALTH_DISTRIBUTION: [
        "drug distribution", "pharmaceutical distribution", "healthcare distribution",
        "medical distributor", "pharmacy distribution", "specialty pharmacy",
    ],
    HealthCategory.HEALTH_IT: [
        "health it", "digital health", "electronic health record", "ehr", "emr",
        "telehealth", "telemedicine", "healthcare software", "health platform",
        "patient engagement", "clinical decision support",
    ],
    HealthCategory.CRO_CDMO: [
        "contract research organization", "cro", "contract development",
        "contract manufacturing", "cdmo", "cmo", "clinical trial services",
        "preclinical services",
    ],
}


# Healthcare jurisdictions are tiered by regulatory quality (FDA/EMA/PMDA path),
# drug pricing stability, reimbursement predictability and IP protection.
...
_TIER_2_JURISDICTIONS = {
    ...
    "uruguay", "turkey", "taiwan",
}


def classify_stage(description: Optional[str], revenue: Optional[float],
                   info: Optional[dict] = None) -> CompanyStage:
    """Classify a healthcare company across its clinical/commercial lifecycle.

    Rough policy:
      * Pre-revenue (<$10M) and no meaningful growth → STARTUP (preclinical / Phase I biotech).
      * Description-keyword hits (Phase III, commercial launch, diversified pharma, …) win first.
      * Otherwise fall back on market cap + profit margin heuristics tuned for healthcare.
    """
    ...
    # Pre-revenue biotech: almost always preclinical / early clinical
    ...
    # Diversified franchise threshold — Big Pharma, integrated healthcare conglomerates
    if mcap >= 150_000_000_000:
        return CompanyStage.PLATFORM
    # Mature commercial healthcare — durable margins on a meaningful book of business
    if profit_margin is not None and profit_margin > 0.15 and mcap >= 10_000_000_000:
        return CompanyStage.MATURE
    # Clinical-stage / late-stage biotech often runs hyper-growth revenue once a product launches
...
def classify_category(description: Optional[str],
                      industry: Optional[str] = None) -> HealthCategory:
    import re
    text = ((description or "") + " " + (industry or "")).lower()
    scores: dict[HealthCategory, int] = {}
...
TechQualityIndicators = HealthQualityIndicators  # pragma: no cover
