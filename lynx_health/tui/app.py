"""Textual UI for Lynx Healthcare Analysis."""
...
class ExplainModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        from lynx_health.metrics.explanations import (
...
class ReportView(VerticalScroll):
    ...
    def add_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        if stage == "profile":
            ...
            self._mount_section("Healthcare Quality", _build_health_quality(report))
...
class LynxHealthApp(PagingAppMixin, App):
    TITLE = "Lynx Healthcare Analysis"
    SUB_TITLE = "Healthcare Fundamental Analysis"
    CSS = """
    #search-dialog {
        width: 60;
        height: auto;
    ...
    def on_mount(self) -> None:
        from lynx_health.tui.themes import register_all_themes, THEME_NAMES
        try:
            register_all_themes(self)
            self.theme = THEME_NAMES[0]  # health-dark
        except Exception:
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield Static(
            "[bold blue]  lynx-health  [/]\n\n"
            "[bold]Healthcare Analysis Tool[/]\n"
    ...
    def _find_parent_section(self, widget) -> str:
        ...
        _TITLE_MAP = {
            ...
            "healthcare quality": "health_quality",
    ...
    def action_cycle_theme(self) -> None:
        from lynx_health.tui.themes import THEME_NAMES
    ...
    def action__ee_lynx(self) -> None:
        from lynx_health.easter import LYNX_ASCII, WOLF_ASCII, BULL_ASCII
        import random
...
def _build_profile_table(r: AnalysisReport) -> DataTable:
    ...
    _r2(t, "Healthcare Category", _s(p.health_category.value if hasattr(p.health_category, "value") else p.health_category))
...
def _build_health_quality(r: AnalysisReport) -> Static:
    """Build a Rich Table for healthcare quality with multi-line cell wrapping."""
    m = r.health_quality
    stage = _get_stage(r)
    tier = _get_tier(r)
    t = RichTable(show_lines=True, border_style="yellow", expand=True)
    ...
    def rel(key: str) -> Relevance:
        return _get_rel(key, tier, "health_quality", stage)
