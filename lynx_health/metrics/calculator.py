"""Healthcare-specialized metrics calculation engine.

All calculations are both tier-aware AND stage-aware. This module computes
traditional financial metrics plus healthcare-specific indicators:

  * R&D Intensity (R&D / revenue) — the primary innovation-commitment signal for pharma & biotech
  * Rule of 40 (revenue growth % + FCF/EBITDA margin %) — applied to health-IT / digital-health SaaS
  * Cash runway & burn (quarterly + annual) — first-order survival metric for clinical-stage biotech
  * Stock-Based Compensation ratios (SBC / revenue, SBC / FCF) — dilution risk for biotech & health IT
  * Revenue per Employee (productivity, very high for insurers & distributors)
  * Deferred Revenue / RPO (forward visibility for health IT SaaS)
  * Goodwill-to-Assets (impairment risk after large biopharma acquisitions)
  * FCF Conversion (earnings quality check for commercial-stage pharma)
  * Magic Number (sales efficiency — only meaningful for health-IT SaaS)
"""
...
from lynx_health.models import (
    ...
    HealthQualityIndicators, ProfitabilityMetrics, ShareStructure,
...
def calc_solvency(
    ...
    # Tangible book value — strip goodwill & intangibles (acquisitions-heavy)
    ...
    # Capex intensity — low for SaaS (<5%), high for cloud/hyperscalers/semi (>15%)
    ...
    # Goodwill-to-assets — impairment risk flag for acquisition-heavy pharma/medtech
...
def calc_growth(
    ...
    # NRR / GRR — Yahoo Finance does not expose customer-level retention.
    # We leave these as None and highlight the data gap in healthcare-quality scoring.
    # (A downstream integration with company IR data would populate these.)
...
def calc_share_structure(
    ...
    # Healthcare companies tend to run larger float counts; thresholds differ from other sectors
...
def calc_health_quality(
    ...
    """Composite quality score 0-100 with healthcare-specific axes.

    Weights:
      - Moat / Gross Margin      (20 pts)
      - Rule of 40 / growth+FCF  (20 pts)
      - Financial position       (15 pts)
      - Dilution / SBC risk      (15 pts)
      - R&D efficiency           (10 pts)
      - Unit economics           (10 pts)
      - Revenue predictability   (10 pts)
    """
    m = HealthQualityIndicators()
    score = 0.0
    max_score = 0.0

    # ── 1. Moat / Gross Margin (20 pts) ───────────────────────────────
    # Healthcare band-lines: branded pharma & devices typically 65-85% GM; generics / distribution
    # 15-30%; managed care essentially has no "gross margin" in the SaaS sense — use operating margin
    # as the anchor for insurer franchises in downstream logic.
    max_score += 20
    gm = profitability.gross_margin if profitability else None
    if gm is not None:
        if gm >= 0.80:
            m.moat_assessment = "Exceptional — >80% gross margin"
            m.moat_type = "Branded pharma / biologics blockbuster economics"
            score += 20
        elif gm >= 0.65:
            m.moat_assessment = "Strong — 65-80% gross margin"
            m.moat_type = "Medical devices, specialty pharma, diagnostics leaders"
            score += 16
        elif gm >= 0.50:
            m.moat_assessment = "Solid — 50-65% gross margin"
            m.moat_type = "Life-sciences tools, health IT, hospital capex players"
            score += 12
        elif gm >= 0.25:
            m.moat_assessment = "Average — 25-50% gross margin"
            m.moat_type = "Generics, medical supplies, outpatient services"
            score += 6
        else:
            m.moat_assessment = "Weak — <25% gross margin"
            m.moat_type = "Drug distribution, managed care, retail pharmacy"
            score += 2
    else:
    ...
    # ── 5. R&D efficiency (10 pts) ──────────────────────────────────
    max_score += 10
    rd = growth.rd_intensity if growth else None
    rev_g = growth.revenue_growth_yoy if growth else None
    if rd is not None:
        if rd >= 0.15 and rev_g is not None and rev_g > 0.20:
            m.rd_efficiency_assessment = f"Strong innovation — R&D {rd*100:.0f}% of rev fuels {rev_g*100:.0f}% growth"
            score += 10
        elif rd >= 0.10:
            m.rd_efficiency_assessment = f"Healthy R&D spend ({rd*100:.0f}% of rev)"
            score += 7
        elif rd >= 0.05:
            m.rd_efficiency_assessment = f"Moderate R&D spend ({rd*100:.0f}% of rev)"
            score += 4
        else:
            m.rd_efficiency_assessment = f"Low R&D ({rd*100:.0f}% of rev) — may lose edge"
            score += 2
    else:
        m.rd_efficiency_assessment = "R&D data unavailable"
        score += 3
    ...
    if m.quality_score >= 75:
        m.competitive_position = "Best-in-Class — Elite Healthcare Franchise"
...
def calc_intrinsic_value(
    ...
    # Method selection for healthcare sector
    if stage == CompanyStage.PLATFORM:
        ...
        iv.primary_method = "DCF"
        iv.secondary_method = "P/FCF Multiple"
    elif stage == CompanyStage.SCALE:
        iv.primary_method = "EV/Gross Profit Multiple"
        iv.secondary_method = "DCF (early-stage)"
    elif stage == CompanyStage.GROWTH:
        iv.primary_method = "EV/Revenue Multiple (Rule-of-40 adj)"
        iv.secondary_method = "Reverse DCF"
    else:  # STARTUP
        iv.primary_method = "Cash Backing + Option Value"
        iv.secondary_method = "Peer EV/Revenue"

    if not statements:
        return iv
    latest = statements[0]

    # DCF for cash-flow-positive healthcare (MATURE / PLATFORM / SCALE with FCF)
    ...
    # EV/sales implied value using a 6x peer multiple (rough healthcare peer benchmark)
...
def calc_market_intelligence(
    ...
    try:
        ...
        if insider_df is not None and not insider_df.empty:
            ...
            # Healthcare nuance: many insider sales happen under 10b5-1 plans. Tag "open-market buying" more strongly.
    ...
    # ── Healthcare benchmark context — XLV as headline benchmark + sub-sector ETF ───
    from lynx_health.models import HealthCategory, classify_category
    _CATEGORY_ETFS = {
        HealthCategory.BIG_PHARMA: [("XPH", "SPDR S&P Pharmaceuticals"),
                                    ("IHE", "iShares US Pharmaceuticals")],
        HealthCategory.BIOTECH: [("XBI", "SPDR S&P Biotech"),
                                 ("IBB", "iShares Biotechnology")],
        HealthCategory.GENERIC_PHARMA: [("XPH", "SPDR S&P Pharmaceuticals"),
                                        ("IHE", "iShares US Pharmaceuticals")],
        HealthCategory.MEDICAL_DEVICES: [("IHI", "iShares US Medical Devices"),
                                         ("XHE", "SPDR S&P Health Care Equipment")],
        HealthCategory.HEALTH_EQUIPMENT: [("IHI", "iShares US Medical Devices"),
                                          ("XHE", "SPDR S&P Health Care Equipment")],
        HealthCategory.HEALTH_PROVIDERS: [("IHF", "iShares US Healthcare Providers"),
                                          ("XHS", "SPDR S&P Health Care Services")],
        HealthCategory.HEALTH_INSURANCE: [("IHF", "iShares US Healthcare Providers"),
                                          ("XHS", "SPDR S&P Health Care Services")],
        HealthCategory.LIFE_SCIENCES_TOOLS: [("XHE", "SPDR S&P Health Care Equipment"),
                                             ("IHI", "iShares US Medical Devices")],
        HealthCategory.HEALTH_DISTRIBUTION: [("IHF", "iShares US Healthcare Providers"),
                                             ("XHS", "SPDR S&P Health Care Services")],
        HealthCategory.HEALTH_IT: [("XHS", "SPDR S&P Health Care Services"),
                                   ("IHF", "iShares US Healthcare Providers")],
        HealthCategory.CRO_CDMO: [("XBI", "SPDR S&P Biotech"),
                                  ("IBB", "iShares Biotechnology")],
        HealthCategory.OTHER: [("XLV", "Health Care Select Sector SPDR"),
    ...
    # Headline healthcare benchmark: XLV (Health Care Select Sector SPDR)
    try:
        qt = yf.Ticker("XLV")
        qi = qt.info or {}
        mi.benchmark_name = "Health Care Select Sector (XLV)"
        mi.benchmark_ticker = "XLV"
    ...
    # Sub-sector ETFs
    try:
        etf_list = _CATEGORY_ETFS.get(category, _CATEGORY_ETFS[HealthCategory.OTHER])
        if len(etf_list) >= 1:
    ...
    # ── Projected SBC dilution (for clinical-stage healthcare) ─────────────
    ...
    # ── Risk warnings (healthcare-specific) ─────────────────────────────────
    warnings: list[str] = []

    if mi.beta and mi.beta > 1.5:
        warnings.append(
            f"Elevated volatility (beta {mi.beta:.1f}) — biotech / specialty pharma often binary around catalysts"
        )
    ...
    if stage in growth_stages and solvency and solvency.total_debt and solvency.total_debt > 0:
        if solvency.net_debt and solvency.net_debt > 0:
            warnings.append("Net debt at clinical / pre-profit stage — refinancing & dilution risk elevated")

    mi.risk_warnings = warnings

    # ── Healthcare disclaimers ──────────────────────────────────────
    disclaimers: list[str] = [
        "Healthcare outcomes are driven by regulatory (FDA/EMA/PMDA) and reimbursement events that can reset equity value abruptly.",
        "Clinical-trial readouts, CRLs and payer coverage decisions are binary-ish catalysts — size positions accordingly.",
        "Patent cliffs, biosimilar / generic entry, and drug-pricing legislation (IRA negotiation, 340B) can impair blockbuster economics.",
    ]
    if stage == CompanyStage.STARTUP:
        disclaimers.append(
            "Preclinical / early-clinical biotech: science quality and cash runway dominate, not financials."
        )
    if stage == CompanyStage.GROWTH:
        disclaimers.append(
            "Clinical-stage companies: Phase III failure is the dominant downside — ~35% historical failure rate at Phase III."
        )
    if category in (HealthCategory.BIOTECH, HealthCategory.CRO_CDMO):
        disclaimers.append(
            "Biotech valuations are reflexive: financing access drives survival when sentiment turns."
        )
    if category == HealthCategory.HEALTH_INSURANCE:
        disclaimers.append(
            "Managed-care earnings hinge on Medical Loss Ratio — a 100bps surprise can move EPS 10-15%."
        )
    if category in (HealthCategory.BIG_PHARMA, HealthCategory.GENERIC_PHARMA):
        disclaimers.append(
            "Large pharma is exposed to loss-of-exclusivity (LOE) on blockbuster franchises — always map the 5-year patent cliff."
        )
    disclaimers.extend([
        "Past clinical results and insider activity do not guarantee future regulatory or commercial success.",
...
calc_mining_quality = calc_health_quality  # pragma: no cover
