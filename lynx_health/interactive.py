"""Interactive prompt-based mode for lynx-health."""
...
from lynx_health.core.news import download_article
from lynx_health.core.reports import download_filing
from lynx_health.display import display_full_report, display_report_stage
from lynx_health.models import AnalysisReport

console = Console()

BANNER = "\n[bold blue]  L Y N X   Healthcare Analysis[/]\n[dim]    Pharma, Biotech, Medical Devices, Managed Care & Life Sciences Tools[/]\n"
...
def run_interactive():
    from lynx_health.core.storage import get_mode, is_testing
    ...
    while True:
        ...
        try:
            console.print(f"\n[bold {prompt_color}]lynx-health[/] ", end="")
            raw = input().strip()
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            from lynx_health import get_about_text; from rich.panel import Panel as P
            a = get_about_text()
            ...
            from lynx_health.metrics.explanations import list_metrics
            ...
            from lynx_health.core.ticker import search_companies, display_search_results
            ...
            try:
                ...
                if isinstance(e, SectorMismatchError):
                    ...
                    console.print(Panel(
                        ...
                        f"[bold red]{e}[/]\n\n"
                        f"[bold blink red]This tool is specialized ONLY for Healthcare.[/]",
            ...
            if current_report:
                from lynx_health.display import _display_health_quality, _display_share_structure, _display_intrinsic_value, _display_market_intelligence
                _display_health_quality(current_report); _display_share_structure(current_report)
            ...
            if current_report and current_report.filings:
                from lynx_health.display import _display_filings; _display_filings(current_report)
            else: console.print("[yellow]No filings available.[/]")
        elif cmd == "news":
            if current_report and current_report.news:
                from lynx_health.display import _display_news; _display_news(current_report)
            ...
            from lynx_health.core.storage import list_cached_tickers, get_mode
            ...
            from lynx_health.core.storage import drop_cache_all, drop_cache_ticker, get_mode
            if arg.lower() == "all":
            ...
            from lynx_health.metrics.explanations import get_section_explanation, SECTION_EXPLANATIONS
            if not arg:
            ...
            from lynx_health.metrics.explanations import get_conclusion_explanation, CONCLUSION_METHODOLOGY
            ...
            from lynx_health.easter import rich_matrix; rich_matrix(console)
        elif cmd == "fortune":
            from lynx_health.easter import rich_fortune; rich_fortune(console)
        elif cmd == "rocket":
            from lynx_health.easter import rich_rocket; rich_rocket(console)
        elif cmd == "lynx":
            from lynx_health.easter import rich_lynx; rich_lynx(console)
        else:
