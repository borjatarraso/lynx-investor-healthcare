...
[bold green]      | |  --/ V \\--   |  |      [/][dim]Healthcare Analysis[/]
[bold green]       \\ \\    ___    / /
        \\ \\---------/ /
         \\___________/[/]
"""
...
[bold cyan]
      ...
      /         \
     |  +-----+  |
     |  |  Rx |  |       [bold white]R x  H E A L T H[/bold white]
     |  +-----+  |       [dim]Biopharma & MedTech Research[/dim]
     |   .===.   |
      \_/     \_/
[/bold cyan]
"""
...
FORTUNE_QUOTES = [
    ...
    '"The best returns in healthcare go to innovators, not the cheapest generics." \u2014 Cathie Wood',
    '"You can\u2019t value a drug you don\u2019t understand." \u2014 John Templeton (adapted)',
    '"A patent cliff is a forecast; execution is destiny." \u2014 industry proverb',
    '"In biotech, capital is the life-support system." \u2014 Vinod Khosla',
    '"The best pharmaceutical is the one that makes the patient better at a price the payer can live with." \u2014 industry proverb',
    '"Invest in companies that help people live longer, better lives \u2014 the rest is just arithmetic." \u2014 anonymous healthcare PM',
]
...
def tk_fireworks(root) -> None:
    ...
    def _animate():
        ...
        particles = alive
        if step[0] == 1:
            for i, msg in enumerate(["\u2605  lynx-health  \u2605", "Healthcare Analysis", "\U0001f680 To the Moon! \U0001f680"]):
...
def tk_rainbow_title(root, count: int = 20) -> None:
    ...
    def _step(remaining, idx):
        if remaining <= 0:
            ...
            return
        try: root.title(f"{random.choice(sparkles)} {colors[idx % len(colors)]} lynx-health {colors[idx % len(colors)]} {random.choice(sparkles)}")
        except tk.TclError: return
