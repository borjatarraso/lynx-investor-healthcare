...
from lynx_health.models import AnalysisReport, CompanyStage, Severity
...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Healthcare  --  {p.name} ({p.ticker})")
    lines.append(f"  Tier: {_ev(p.tier)}  |  Stage: {_ev(p.stage)}")
    lines.append(f"  Healthcare Category: {_ev(p.health_category)}  |  Jurisdiction: {_ev(p.jurisdiction_tier)}")
    ...
    if report.health_quality:
        mq = report.health_quality
        lines += _section("HEALTHCARE QUALITY INDICATORS")
        _fields = [
    ...
    if report.market_intelligence:
        ...
        if mi.benchmark_name or mi.sector_etf_name:
            lines.append("")
            lines.append("  Healthcare Benchmark & Sector Context:")
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Healthcare Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Healthcare (Lince Investor Suite)")
    lines.append("=" * W)
