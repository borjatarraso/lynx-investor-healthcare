"""Main analysis orchestrator for Healthcare companies."""
...
from lynx_health.core.fetcher import fetch_company_profile, fetch_financial_statements, fetch_info
from lynx_health.core.news import fetch_all_news
from lynx_health.core.reports import download_top_filings, fetch_sec_filings
from lynx_health.core.storage import get_cache_age_hours, has_cache, load_cached_report, save_analysis_report
from lynx_health.core.ticker import resolve_identifier
from lynx_health.metrics.calculator import (
    calc_efficiency, calc_growth, calc_intrinsic_value, calc_market_intelligence,
    calc_health_quality, calc_profitability, calc_share_structure, calc_solvency,
    calc_valuation,
)
from lynx_health.models import (
    ...
    HealthQualityIndicators, NewsArticle,
...
_ALLOWED_SECTORS = {"healthcare", "health care"}
_ALLOWED_INDUSTRIES = {
    ...
    "pharmaceutical retailers", "long-term care facilities",
    "pharmaceutical preparations",
}


class SectorMismatchError(Exception):
    """Raised when a company does not belong to Healthcare."""
    pass


def _validate_sector(profile: CompanyProfile) -> None:
    """Check if the company belongs to Healthcare.
    ...
    # Healthcare-specific terms (word-boundary matched) used to detect
    # healthcare business when the Yahoo sector field is missing or mislabeled.
    healthcare_specific = [
        r"\bpharmaceutical\b", r"\bbiopharmaceutical\b", r"\bbiotech(nology)?\b",
        r"\bclinical.?stage\b", r"\bphase [123iv]+\b", r"\bpreclinical\b",
        r"\bfda\b.*\b(approval|submission|new drug)\b",
        r"\bmonoclonal antibody\b", r"\bgene therapy\b", r"\bcell therapy\b",
        r"\bmrna\b", r"\bcrispr\b", r"\bgene editing\b", r"\bvaccine\b",
        r"\bmedical devices?\b", r"\bmedical equipment\b", r"\bdiagnostic\b",
        r"\bin vitro diagnostic\b", r"\bgenomic sequencing\b",
        r"\bhospital operator\b", r"\bhealth system\b", r"\boutpatient\b",
        r"\bdialysis\b", r"\bmanaged care\b", r"\bhealth insurance\b",
        r"\bmedicare advantage\b", r"\bpharmacy benefit\b",
        r"\bcontract research organization\b", r"\bcontract development\b",
        r"\bcdmo\b", r"\bcro\b",
        r"\bmedical distribution\b", r"\bspecialty pharmacy\b",
        r"\belectronic health record\b", r"\bdigital health\b", r"\btelehealth\b",
    ]
    if any(re.search(pattern, desc) for pattern in healthcare_specific):
        return
    ...
    message = (
        ...
        f"Lynx Healthcare Analysis is specialized exclusively for:\n"
        f"  - Large / diversified pharmaceuticals & biologics\n"
        f"  - Biotechnology (clinical-stage, launch, commercial)\n"
        f"  - Generic & specialty pharma\n"
        f"  - Medical devices & healthcare equipment\n"
        f"  - Healthcare providers (hospitals, dialysis, outpatient)\n"
        f"  - Managed-care / health insurance\n"
        f"  - Life sciences tools, diagnostics & CROs/CDMOs\n"
        f"  - Drug & medical distribution\n"
        f"  - Health IT / digital health\n\n"
        f"For other sectors, use the matching Lynx specialist "
        f"(Information Technology, Basic Materials, Energy, Financials, etc.)."
    )
    message += format_agent_suggestion(
        profile, current_agent="lynx-investor-healthcare",
    )
...
def run_progressive_analysis(
    ...
    profile.stage = stage
    profile.health_category = classify_category(profile.description, profile.industry)
    ...
    # Validate sector — refuse to analyze non-healthcare companies
    _validate_sector(profile)

    if profile.health_category.value != "Other Healthcare":
        console.print(f"[cyan]Healthcare Category:[/] {profile.health_category.value}")
...
def _dict_to_report(d: dict) -> AnalysisReport:
    ...
    return AnalysisReport(
        ...
        health_quality=_maybe(HealthQualityIndicators, "health_quality"),
