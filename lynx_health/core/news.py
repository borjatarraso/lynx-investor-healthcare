"""News fetching for Healthcare companies."""
...
from lynx_health.core.storage import get_news_dir, save_json, save_text
from lynx_health.models import NewsArticle
...
def fetch_news_rss(company_name: str, ticker: str) -> list[NewsArticle]:
    query = f"{company_name} {ticker} healthcare stock"
...
def download_article(ticker: str, article: NewsArticle) -> Optional[str]:
    ...
    try:
        resp = requests.get(article.url, timeout=15, allow_redirects=False, headers={"User-Agent": "Mozilla/5.0 (compatible; LynxHealth/0.1)"})
