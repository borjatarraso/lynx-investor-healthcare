"""Healthcare-focused report synthesis engine.

Generates the weighted overall score + verdict + strengths/risks for a
Healthcare company. Weights adapt by both tier and healthcare lifecycle
stage. The screening checklist covers the 10 most important healthcare
quality criteria — runway, dilution, R&D intensity, moat (gross margin),
jurisdiction quality, insider alignment, and growth.
"""
...
from lynx_health.models import AnalysisConclusion, AnalysisReport, CompanyStage, CompanyTier, JurisdictionTier
...
# Weights: (valuation, profitability, solvency, growth, health_quality)
_WEIGHTS = {
...
def generate_conclusion(report: AnalysisReport) -> AnalysisConclusion:
    ...
    quality_score = _safe(report.health_quality.quality_score) if report.health_quality else 0
    ...
    c.category_scores = {"valuation": round(val_score, 1), "profitability": round(prof_score, 1),
                         ...
                         "health_quality": round(quality_score, 1)}
    ...
    c.screening_checklist = _health_screening(report)
    return c
...
def _score_valuation(r: AnalysisReport) -> float:
    ...
    # EV / Gross Profit — the headline healthcare anchor
    ...
    # Traditional P/E for mature healthcare only
...
def _score_profitability(r: AnalysisReport) -> float:
    ...
    # Net margin / FCF margin for mature healthcare
    if stage in (CompanyStage.MATURE, CompanyStage.PLATFORM):
...
def _health_screening(r: AnalysisReport) -> dict:
    """Ten-point Healthcare screening checklist."""
    checks: dict = {}
    p, s, g, ss, tq = r.profitability, r.solvency, r.growth, r.share_structure, r.health_quality
    stage = r.profile.stage
...
def _build_summaries(r: AnalysisReport) -> dict[str, str]:
    ...
    summaries["health_quality"] = (r.health_quality.competitive_position or "N/A") if r.health_quality else "N/A"
    return summaries
...
def _find_risks(r: AnalysisReport) -> list[str]:
    ...
    if r.profile.stage == CompanyStage.STARTUP:
        risks.append("Early-stage biotech — product-market-fit risk dominates")
    return risks[:6]
...
def _tier_note(tier: CompanyTier) -> str:
    return {
        CompanyTier.MEGA: "Mega-cap: diversified franchise — DCF, ROIC compounding, patent-cliff coverage and pipeline replenishment dominate.",
        CompanyTier.LARGE: "Large-cap: balanced growth + profitability. EV/EBITDA, P/FCF and R&D productivity are primary anchors.",
        CompanyTier.MID: "Mid-cap: watch operating-margin trajectory, launch execution, and biosimilar / patent-cliff exposure.",
        CompanyTier.SMALL: "Small-cap healthcare: execution & financing risk elevated. Runway, SBC dilution and regulatory milestones critical.",
        CompanyTier.MICRO: "Micro-cap: survival metrics dominate for biotech; operational quality dominates for micro-cap services. Runway, dilution, insider alignment are must-checks.",
        CompanyTier.NANO: "Nano-cap: highly speculative. Cash-backing floor and pipeline / contract quality are the only durable anchors.",
    }.get(tier, "")


def _stage_note(stage: CompanyStage) -> str:
    return {
        CompanyStage.PLATFORM: "Diversified Healthcare Franchise: blockbuster portfolio + cash generation compound for decades. DCF & ROIC primary; watch patent-cliff coverage.",
        CompanyStage.MATURE: "Commercial / Cash-Generative: predictable FCF, buybacks, dividends. EV/EBITDA and P/FCF primary; monitor LOE calendar.",
        CompanyStage.SCALE: "Launch / Scale-Up: commercial ramp on first approved product(s). Watch uptake curves, payer coverage, and margin trajectory.",
        CompanyStage.GROWTH: "Clinical / Hyper-Growth: near-term catalysts dominate. Cash runway to next readout, trial design quality, and SBC overhang are the key gates.",
        CompanyStage.STARTUP: "Preclinical / Early-Stage Biotech: science quality, cash-backing floor, and scientific-founder alignment dominate — not financials.",
    }.get(stage, "")
