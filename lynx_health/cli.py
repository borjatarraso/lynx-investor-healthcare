"""Command-line interface for lynx-health."""
...
from lynx_health import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-health",
        description="Lynx Healthcare Analysis — Big Pharma, Biotech, Medical Devices, Healthcare Services, Managed Care & Life Sciences Tools fundamental analysis tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-health -p JNJ                             Diversified healthcare franchise\n"
            "  lynx-health -p LLY                             Mega-cap pharma (GLP-1 leader)\n"
            "  lynx-health -p MRNA                            Clinical/commercial biotech\n"
            "  lynx-health -p ISRG --refresh                  Force fresh data (Intuitive Surgical)\n"
            '  lynx-health -p "Vertex Pharmaceuticals"        Search by name\n'
            "  lynx-health -p -i                              Interactive mode\n"
            "  lynx-health -t UNH                             Testing mode (UnitedHealth)\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_health.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_health import get_about_text
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_health.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_health.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_health.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_health.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_health.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_health.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_health.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_health.core.storage import list_cached_tickers, get_mode
